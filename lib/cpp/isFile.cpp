		...
		// public
			...
			void isFileSync(const v8::FunctionCallbackInfo<v8::Value>& args) {
				...
				v8::Isolate *isolate = args.GetIsolate();
					...
					// params treatment
					if (0 >= args.Length()) {
						isolate->ThrowException(v8::Exception::ReferenceError(v8::String::NewFromUtf8(isolate, "missing 'filename' argument")));
							...
							isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "'filename' argument is not a string")));
						}
					else {
			...
			void isFileProm(const v8::FunctionCallbackInfo<v8::Value>& args) {
				...
				if (0 >= args.Length()) {
					local->Reject(v8::Exception::ReferenceError(v8::String::NewFromUtf8(isolate, "missing 'filename' argument")));
				}
					else if (!args[0]->IsString()) {
						local->Reject(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "'filename' argument is not a string")));
					}
				else {

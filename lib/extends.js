				...
				if (0 >= files.length) {
					...
					return fs.isFileProm(file).then((exists) => {
						...
						return _readContentProm(
							files, encoding, separator, ("" === content) ? filecontent : content + separator + filecontent
						);
		...
		function _emptyDirectoryProm(dir, files) {
			...
			return Promise.resolve().then(() => {
				...
				if (0 >= files.length) {
					...
					let file = files.pop(), curPath = path.join(dir, file);
	...
	// directoryFilesToString
		...
		fs.directoryFilesToStringSync = (dir, encoding, separator) => {
			...
			if ("string" !== typeof dir) {
				...
				let result = [], files = fs.readdirSync(dir);
